...
/**
 * Classe che rappresenta l'obiettivo che riguarda l'utilizzare il GeigerCounter.
 *
 ...
 * @see Objective
 */
public class UseGeigerCounter extends Objective {

    /**
     * Costruttore della classe UseGeigerCounter.
     * 
     * @param id numero obiettivo.
     * @param db il database dal quale leggere le varie informazioni dell'obiettivo UseGeigerCounter.
     * 
     * @throws SQLException per eventuali problemi con il database o l'SQL.
     */
    public UseGeigerCounter(int id, DB db) throws SQLException {
        super(id, db);
    }
    ...
    public void checkCompleted(List<Item> items, GameFrame gf) {
        ...
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getName().toLowerCase().equals("contatore di geiger")) {
                item = items.get(i);
