...
/**
 * Classe che rappresenta l'obiettivo che riguarda il trovare ed usare l'item Bandage.
 *
 ...
 * @see Objective
 */
public class UseBandage extends Objective {

    /**
     * Costruttore della classe UseBandage.
     * 
     * @param id numero obiettivo.
     * @param db il database dal quale leggere le varie informazioni dell'obiettivo UseBandage.
     * 
     * @throws SQLException per eventuali problemi con il database o l'SQL. 
     */
    public UseBandage(int id, DB db) throws SQLException {
        super(id, db);
    }
    ...
    public void checkCompleted(List<Item> items, GameFrame gf) {
        ...
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getName().toLowerCase().equals("bende")) {
                item = items.get(i);
