...
/**
 * Classe che rappresenta l'obiettivo che riguarda il riparare l'Electrical
 * Panel.
 *
 ...
 * @see Objective
 */
public class LightObjective extends Objective {

    /**
     * Costruttore della classe LightObjective.
     *
     * @param id numero obiettivo.
     * @param db il database dal quale leggere le varie informazioni
     * dell'obiettivo LightObjective.
     * 
     * @throws SQLException per eventuali problemi con il database o l'SQL.
     */
    public LightObjective(int id, DB db) throws SQLException {
        super(id, db);
    }
    ...
    public void checkCompleted(List<Item> items, GameFrame gf) {
        ...
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getName().toLowerCase().equals("quadro elettrico")) {
                item = items.get(i);
