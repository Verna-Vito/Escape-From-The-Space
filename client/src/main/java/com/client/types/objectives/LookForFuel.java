...
/**
 * Classe che rappresenta l'obiettivo che riguarda il trovare l'item Fuel.
 *
 ...
 * @see Objective
 */
public class LookForFuel extends Objective {

    /**
     * Costruttore della classe LookForFuel.
     *
     * @param id numero obiettivo.
     * @param db il database dal quale leggere le varie informazioni
     * dell'obiettivo LookForFuel.
     * 
     * @throws SQLException per eventuali problemi con il database o l'SQL.
     */
    public LookForFuel(int id, DB db) throws SQLException {
        super(id, db);
    }
    ...
    public void checkCompleted(List<Item> items, GameFrame gf) {
        ...
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getName().toLowerCase().equals("carburante")) {
                item = items.get(i);
                break;
            }
        }
