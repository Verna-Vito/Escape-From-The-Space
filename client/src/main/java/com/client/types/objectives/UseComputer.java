...
/**
 * Classe che rappresenta l'obiettivo che riguarda l'utilizzo del Computer.
 *
 ...
 * @see Objective
 */
public class UseComputer extends Objective {

    /**
     * Costruttore della classe UseComputer.
     * 
     * @param id numero obiettivo.
     * @param db il database dal quale leggere le varie informazioni dell'obiettivo UseComputer.
     * 
     * @throws SQLException per eventuali problemi con il database o l'SQL. 
     */
    public UseComputer(int id, DB db) throws SQLException {
        super(id, db);
    }
    ...
    public void checkCompleted(List<Item> items, GameFrame gf) {
        ...
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getName().toLowerCase().equals("computer di bordo")) {
                item = items.get(i);
