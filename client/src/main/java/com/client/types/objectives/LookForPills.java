...
/**
 * Classe che rappresenta l'obiettivo che riguarda il trovare l'item Pills.
 *
 ...
 * @see Objective
 */
public class LookForPills extends Objective {

    /**
     * Costruttore della classe LookForPills.
     *
     * @param id numero obiettivo.
     * @param db il database dal quale leggere le varie informazioni
     * dell'obiettivo LookForPills.
     * 
     * @throws SQLException per eventuali problemi con il database o l'SQL.
     */
    public LookForPills(int id, DB db) throws SQLException {
        super(id, db);
    }
    ...
    public void checkCompleted(List<Item> items, GameFrame gf) {
        ...
        if (!gf.getInventory().isEmpty()) {
            for (int i = 0; i < gf.getInventory().size(); i++) {
                if (gf.getInventory().get(i).getName().toLowerCase().equals("pillole")) {
